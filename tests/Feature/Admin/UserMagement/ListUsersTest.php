...
test('it should be able to filter by name and email', function () {
...
test('it should be able to list deleted users', function () {
    $admin       = User::factory()->admin()->create(['name' => 'Joe Doe', 'email' => 'admin@gamil.com']);
    $deletedUser = User::factory()->count(2)->create(['deleted_at' => now()]);
...
test('it shoule be able sort by name', function () {
    $admin    = User::factory()->admin()->create(['name' => 'Joe Doe', 'email' => 'admin@gamil.com']);
    $nonAdmin = User::factory()->withPermission(Can::TESTING)->create(['name' => 'Mario', 'email' => 'little_guy@gmail.com']);
...
test('it should be able to paginate the result', function () {
    $admin = User::factory()->admin()->create(['name' => 'Joe Doe', 'email' => 'admin@gamil.com']);
    ...
    Livewire::test(Admin\Users\Index::class)
        ->assertSet('users', function (LengthAwarePaginator $users) {
            expect($users)
                ->toHaveCount(15); // 10 + 1 for the paginator
